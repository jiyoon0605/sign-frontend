...
const LoginPresenter:React.FC<Props>=({email,setEmail,password,setPassword,onHandleClick})=>{
    ...
    return <S.Container>
        <S.Title>로그인</S.Title>
        <S.InputField type="text" placeholder="이메일" value={email} onChange={e=>setEmail(e.target.value)}/>
        <S.InputField placeholder="비밀번호" type="password" 
        value={password} onChange={e=>setPassword(e.target.value)} />
        <S.SubmitButton type="submit" onClick={(e)=>{e.preventDefault();onHandleClick();}} >로그인</S.SubmitButton>
